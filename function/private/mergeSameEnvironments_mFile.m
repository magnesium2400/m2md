function newstr = mergeSameEnvironments_mFile(str,environment)
%  Copyright 2020 The MathWorks, Inc.

% str is a vector of string
% start index of a enviroment
% idx_start = startsWith(str,"\begin{"+environment+"}");
temp = regexp(str,"\s{0,}\\begin{"+environment+"}");
for ii = 1:length(temp); idx_start(ii) = isequal(temp{ii}, 1); end %#ok<AGROW> 
% start index of a enviroment
idx_end = endsWith(str,"\end{"+environment+"}");

% for each \begin and \end pair needs to be in one string
start_lstlisting = find(idx_start);
end_lstlisting = find(idx_end);

% check if the number of \begin and that of \end match.
