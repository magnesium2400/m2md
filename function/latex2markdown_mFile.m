function mdfile = latex2markdown_mFile(filename,options)
...
arguments
    ...
    options.ToC logical = true
    options.verbose logical = false
end

% Latex filename
[filepath,name,ext] = fileparts(filename);
[outpath,outname,outext] = fileparts(options.outputfilename);
...
% function declaration. So, take the second subsection (becuase this will be
% after the TOC) and use it to make H1.
eraseFirst = false;
if ~contains(str, "\section")
    [startI, endI] = regexp(str, "\\subsection\*{([^{}]+)}");
    h = extractBetween(str, startI(2), endI(2));
    str = eraseBetween(str, startI(2), endI(2));
...
k = strfind(str, "\subsection*{ENDPUBLISH}");
try str = extractBefore(str, k(1)); catch; end
str = regexprep(str, "\\section\*{([^{}]+)}","# $1");
str = regexprep(str, "\\subsection\*{([^{}]+)}","# $1");

%% ToC will alswyas be created by PUBLISH 
% \matlabtitle{タイトル}
...
% % % % % 
% % % % % 
% % % % % % temp = extractBetween(str, "# Contents", "ENDPUBLISH");
% % % % % % if (isempty(temp))
% % % % % %     disp('empty');
% % % % % %     temp = extractBetween(str,"# Contents", "\end{itemize}");
% % % % % % end
% % % % % % % k = strfind(temp, "\item ");
% % % % % % % temp = extractAfter(temp, k(1)-1);
% % % % % % % toc_md = regexprep(temp, "\s{0,}\\item", "\n-");
...
% entry
if eraseFirst
    idx = strfind(str, "\item ");
    contentsPosition = strfind(str, "# Contents");
...
str = mergeSameEnvironments_mFile(str,"lstlisting");
str = mergeSameEnvironments_mFile(str,"verbatim");
str = mergeSameEnvironments_mFile(str,"matlabcode");
str = mergeSameEnvironments_mFile(str,"matlaboutput");
str = mergeSameEnvironments_mFile(str,"matlabtableoutput");
str = mergeSameEnvironments_mFile(str,"matlabsymbolicoutput");

%% Let's convert latex to markdown
% 1: Process parts that require literal output.
[str, idxLiteral] = processLiteralOutput_mFile(str);
